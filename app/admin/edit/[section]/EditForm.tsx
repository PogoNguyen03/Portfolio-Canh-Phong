...
const labelClasses =
  "text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider ml-1 mb-1 block";
const cardClasses =
  "bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:shadow-md hover:border-blue-300 dark:hover:border-blue-500/50 transition-all p-6";
...
export function EditForm({
  ...
  const handleEditClick = (item = {}) => {
    ...
    if (isSkills) {
      Object.keys(itemToEdit).forEach((key) => {
        itemToEdit[key] = Array.isArray(initialData[key])
          ? initialData[key].join(", ")
          : initialData[key];
      });
    } else {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (isSkills) {
      ...
      Object.keys(editItem).forEach((key) => {
        dataToSend[key] = String(editItem[key])
          .split(",")
          .map((s: string) => s.trim())
      ...
      dataToSend.description =
        typeof dataToSend.description === "string"
          ? dataToSend.description
              .split("\n")
              .map((s: string) => s.trim())
              .filter((s: string) => s.length > 0)
  ...
  const handleDelete = async (id: number) => {
    if (!deleteAction || !window.confirm("Bạn có chắc chắn muốn xóa mục này?"))
      return;
  ...
  const renderFormContent = () => {
      ...
      return (
        <div className="grid grid-cols-1 gap-6">
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-xl text-sm border border-blue-100 dark:border-blue-800 flex items-start gap-3">
            ...
            <div>
              <strong>Hướng dẫn:</strong> Nhập các kỹ năng cách nhau bởi dấu
              phẩy.
              <br />
              Ví dụ: <code>React, Next.js, Node.js</code>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Object.keys(initialData).map((key) => (
              <InputField
                ...
                name={key}
                value={editItem[key]}
                onChange={(e: any) => handleSkillChange(key, e.target.value)}
                isTextArea
              />
            ))}
          </div>
    ...
    const fields = Object.keys(getInitialItemState(sectionName)).filter(
      (key) => !["id", "images", "avatar", "cvPath", "visibility"].includes(key)
    );
        ...
        {isSingleObject && !isSkills && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-700">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="w-full sm:w-auto px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-lg text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition flex items-center justify-center gap-2"
                  >
                    {uploading ? (
                    ...
                    {editItem.avatar ? "Đổi ảnh" : "Tải ảnh lên"}
                  </button>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-4">
                <div className="w-20 h-20 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 flex items-center justify-center border border-red-100 dark:border-red-900/30">
                  <FileText size={32} />
                </div>
                <div className="flex-1">
                  <input
                  ...
                  >
                    ...
                    {editItem.cvPath ? "Cập nhật CV" : "Tải CV lên"}
                  </button>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {editItem.images &&
                  editItem.images.map((img: string, idx: number) => (
                    <div
                      key={idx}
                      className="relative group aspect-video rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700"
                    >
                      <img
                        src={img}
                        className="w-full h-full object-cover"
                ...
                <button
                  ...
                  disabled={uploading}
                  className="flex flex-col items-center justify-center aspect-video rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-blue-500 dark:hover:border-blue-400 bg-white dark:bg-slate-800/50 transition-colors text-slate-400 hover:text-blue-500"
                >
                  {uploading ? (
                ...
                <input
                  ...
                  onChange={(e) =>
                    e.target.files?.[0] &&
                    handleFileUpload(e.target.files[0], "projectImage")
                  }
                  className="hidden"
                />
              </div>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      {message && (
        <div
          className={`p-4 rounded-xl flex items-start gap-3 shadow-lg border animate-in slide-in-from-top-2 duration-300 ${
          ...
          <div className="p-6 md:p-8">
            <div className="flex justify-between items-center mb-8 pb-6 border-b border-slate-100 dark:border-slate-800">
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 ml-12">
              ...
              {!isSingleObject && (
                <button
                  ...
                  className="p-2 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition text-slate-500 dark:text-slate-400"
                >
                  <X size={20} />
                </button>
              )}
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mt-10 flex gap-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                <button
                  type="submit"
                  disabled={uploading}
                  className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3.5 rounded-xl font-bold shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all transform active:scale-[0.98]"
                >
                  {uploading ? (
                    <Loader2 className="animate-spin" size={20} />
                  ) : (
                ...
                {!isSingleObject && (
                  <button
                    type="button"
                    onClick={handleCancel}
                    className="px-6 py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition font-medium border border-transparent dark:border-slate-700"
                  >
                    Hủy Bỏ
                  </button>
                )}
      ...
      {!isEditing && !isSingleObject && (
        <div className="flex justify-end">
          <button
            ...
            className="flex items-center gap-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-3 rounded-xl font-bold hover:bg-slate-800 dark:hover:bg-slate-100 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1"
          >
      ...
      {!isEditing && Array.isArray(listData) && listData.length > 0 && (
        <div className="space-y-6">
          <h3 className="text-xl font-bold text-slate-800 dark:text-white px-1 border-l-4 border-blue-500 pl-3">
            Danh sách hiện tại ({listData.length})
          </h3>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="space-y-1.5 flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      {item.period && (
                        <span className="text-xs font-semibold px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md border border-slate-200 dark:border-slate-600">
                          {item.period}
                        </span>
                      )}
                    </div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 max-w-2xl leading-relaxed">
                  ...
                  <div className="flex items-center gap-2 self-end md:self-center shrink-0">
                    <button
                      ...
                      className="p-2.5 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-xl transition border border-blue-100 dark:border-blue-900/50"
                      title="Sửa"
                    >
                      <Edit size={18} />
                    </button>
                    <button
                      ...
                      className="p-2.5 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-xl transition border border-red-100 dark:border-red-900/50"
                      title="Xóa"
                    >
                      <Trash2 size={18} />
                    </button>
