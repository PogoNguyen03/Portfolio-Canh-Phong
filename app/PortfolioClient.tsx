...
//Dùng thay thể cho blur
const BackgroundEffects = React.memo(() => (
  <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
    <div className="absolute inset-0 bg-grid-pattern opacity-[0.1]" />
    <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-blue-500/10 rounded-full blur-[60px]" />
...
const Typewriter = ({
  ...
  useEffect(() => {
    ...
    const controls = animate(count, currentText.length, {
      ...
      ease: "linear",
      onUpdate: (latest) =>
        setDisplayedText(currentText.slice(0, Math.round(latest))),
      onComplete: () => {
        setTimeout(() => {
          animate(count, 0, {
            type: "tween",
  ...
  return (
    ...
    <span className="inline-block relative h-[1.2em] leading-none align-baseline">
      ...
      {/* Kỹ thuật giữ chiều cao dòng tối thiểu */}
...
const ExperienceItem = React.memo(
  ({ exp, index }: { exp: any; index: number }) => {
    return (
      <motion.div
        initial={{ opacity: 0, x: -20 }}
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg hover:border-blue-300 dark:hover:border-blue-700 transition-all relative z-10">
              ...
              <ul className="space-y-3">
                {exp.description.map((desc: string, i: number) => (
                  <li
                    key={i}
                    className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed flex items-start gap-3"
                  >
...
const ProjectCard = React.memo(
  ({ project, index }: { project: any; index: number }) => {
    ...
    return (
      <motion.div
        ...
        className={`group relative flex flex-col-reverse gap-6 md:gap-8 md:items-center ${
      ...
      >
        ...
        <div className="flex-1 relative z-10">
          ...
          >
            ...
            <span
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${
                isPrivate
          ...
          <h3
            className={`text-2xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-4 md:mb-6 ${
          ...
          <div
            className={`flex flex-wrap gap-4 mb-4 text-sm font-medium ${
              isEven ? "justify-start" : "md:justify-end justify-start"
            }`}
          >
          ...
          <div
            className={`relative transition-transform hover:-translate-y-1 duration-300 md:p-8 md:bg-white md:dark:bg-slate-900/90 md:rounded-2xl md:shadow-xl md:border md:border-slate-100 md:dark:border-slate-800 md:backdrop-blur-sm ${
              isEven ? "md:mr-0 md:-mr-16" : "md:ml-0 md:-ml-16"
            }`}
          >
            <p
              className="text-slate-600 dark:text-slate-400 leading-relaxed text-sm md:text-base line-clamp-4"
              title={project.overview} // Tooltip hiện full khi hover
            >
              {truncateText(project.overview, 220)}
            </p>
          </div>
          <div
          ...
          >
            {stackList.map((tech: string, i: number) => (
              <span
                key={i}
                className="text-xs font-bold px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-full font-mono"
              >
                {tech}
              </span>
            ))}
          ...
          >
            ...
            {project.demo ? (
              ...
              <div
                className="flex items-center gap-2 text-slate-400 dark:text-slate-500 font-bold text-sm cursor-help"
                title="Dự án này hiện chưa có link demo công khai"
              >
            ...
            <div
              className={`hidden md:block absolute -bottom-10 -z-10 text-[180px] font-black text-slate-100 dark:text-slate-800/30 leading-none select-none pointer-events-none transition-all duration-500 ${
        ...
        <div className="flex-[1.2] relative group/img w-full">
          ...
          <TiltCard>
            <Link
              href={`/project/${project.id}`}
              onClick={handleViewDetail}
              className="block relative rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800 aspect-video bg-slate-100 dark:bg-slate-900"
            >
              {coverImage ? (
    ...
    <section id="contact" className="mb-32 scroll-mt-28">
      <div className="text-center mb-16">
        ...
        <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">
          Interested in working together? Drop me a message or contact me
          directly!
        </p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm p-8 rounded-3xl border border-slate-200 dark:border-slate-800 h-full flex flex-col justify-center">
            ...
            <div className="space-y-8">
              ...
              >
                <div className="p-4 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-2xl group-hover:scale-110 transition-transform">
                  <Phone size={24} />
                </div>
                <div>
                  <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
                  ...
                  <p className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">
                    {personalInfo.phone}
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors break-all">
                    {personalInfo.email}
                  </p>
                </div>
              </div>
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-800 border dark:border-slate-700 outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white"
              />
              <input
                name="email"
                type="email"
            ...
            <textarea
              ...
              required
              rows={4}
              className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-800 border dark:border-slate-700 outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white resize-none"
            ></textarea>

            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-500 dark:text-slate-400">
                Attach documents (CV, JD...)
              </label>
              <input
                name="files"
                type="file"
                multiple
                className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-slate-800 dark:file:text-blue-400"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full py-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-500/20"
            >
              {isPending ? "Sending..." : "Send to Gmail"}
            </button>
          </form>
        </motion.div>
      </div>
  ...
  return (
    <>
      ...
      <div className="min-h-screen font-sans selection:bg-blue-500/30 selection:text-blue-900 dark:selection:text-blue-200 overflow-x-hidden relative transition-colors duration-500">
        ...
        <CustomCursor />
        <motion.div
          className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-600 to-cyan-400 origin-left z-[70] hidden md:block"
          style={{ scaleX }}
        />
        ...
        <div className="hidden md:flex fixed top-6 left-0 right-0 justify-center z-50 px-4 pointer-events-none">
          <div className="pointer-events-auto flex items-center gap-4">
            <motion.nav
              ...
              className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/50 shadow-lg shadow-slate-200/50 dark:shadow-black/50 rounded-full px-6 py-3 flex items-center gap-1"
            >
              <span className="font-bold text-slate-800 dark:text-white mr-4">
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="fixed inset-0 z-[100] bg-white dark:bg-slate-950 flex flex-col md:hidden"
            >
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-8">
                {navItems.map((item, index) => (
                  <motion.a
                    ...
                    transition={{ delay: index * 0.1 }}
                    className="text-3xl font-bold text-slate-800 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400"
                  >
                    {item}
                  </motion.a>
                ))}
        ...
        <motion.button
          onClick={toggleTheme}
          whileTap={{ scale: 0.9 }}
          className="md:hidden fixed bottom-6 right-6 z-[60] p-4 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md rounded-full text-slate-800 dark:text-yellow-400 shadow-xl border border-slate-200 dark:border-slate-700"
        >
        ...
        <main className="max-w-6xl mx-auto px-4 md:px-8 pb-1 pt-32 relative z-10">
          ...
          <section
            id="about"
            className="min-h-[85vh] flex flex-col justify-center mb-20 scroll-mt-32"
          >
        ...
        <footer className="py-12 text-center text-slate-400 bg-white/50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800 backdrop-blur-sm">
          <div className="max-w-6xl mx-auto px-4">
            <p className="mb-4">
              © 2025 {personalInfo.name}. Built with Next.js & Tailwind.
            </p>
